/*
  ...
  1. New Tables
    ...
    - content_updates: Audit log for content changes

  2. Functions
    - is_admin: Check if a user is an admin
    - add_admin: Add a new admin user
    - log_content_update: Log content changes
...
CREATE OR REPLACE FUNCTION trigger_log_content_changes()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  IF TG_OP = 'INSERT' THEN
    PERFORM log_content_update(TG_TABLE_NAME, NEW.id, 'insert', row_to_json(NEW)::jsonb);
  ELSIF TG_OP = 'UPDATE' THEN
    PERFORM log_content_update(TG_TABLE_NAME, NEW.id, 'update', jsonb_build_object(
      'old', row_to_json(OLD)::jsonb,
      'new', row_to_json(NEW)::jsonb
    ));
  ELSIF TG_OP = 'DELETE' THEN
    PERFORM log_content_update(TG_TABLE_NAME, OLD.id, 'delete', row_to_json(OLD)::jsonb);
  END IF;
  RETURN NULL;
END;
$$;
...
FOR EACH ROW EXECUTE FUNCTION trigger_log_content_changes();
