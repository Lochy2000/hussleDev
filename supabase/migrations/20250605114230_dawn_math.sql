-- Drop all existing policies and tables to start fresh
DO $$ 
BEGIN
...
CREATE POLICY "hustle_read_policy" ON hustles
FOR SELECT USING (
  auth.uid() = user_id OR 
  EXISTS (
...
CREATE POLICY "hustle_write_policy" ON hustles
...
CREATE POLICY "hustle_update_policy" ON hustles
FOR UPDATE USING (
  auth.uid() = user_id OR 
  EXISTS (
    SELECT 1 FROM admin_users 
...
CREATE POLICY "hustle_delete_policy" ON hustles
FOR DELETE USING (
  auth.uid() = user_id OR 
  EXISTS (
    SELECT 1 FROM admin_users 
